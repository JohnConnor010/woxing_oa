...
{
    public partial class CTR_Modules : System.Web.UI.Page
    {
        ...
        private void pageInit(bool start)
        {
            for(int i=0;i<WX.CRM.Customer_Temp.TypeStr.Length;i++)
                DropDownList1.Items.Add(new ListItem(WX.CRM.Customer_Temp.TypeStr[i],i.ToString()));
            string sql = "select * from CRM_Customer_Temp where Type="+DropDownList1.SelectedValue;
