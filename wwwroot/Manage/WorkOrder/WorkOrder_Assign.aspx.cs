...
{
    public partial class WorkOrder_Assign : System.Web.UI.Page
    {
        ...
        {
            Label1.Text = "";
            System.Data.DataTable dt = WX.WorkOrder.Order.GetAssign2Tables("1,7,8", this.CurUser.UserModel.DepartmentID.ToString());
            
            System.Data.DataTable dt2 = WX.WorkOrder.Order.GetAssignTables("3,4,5,6", this.CurUser.UserModel.DepartmentID.ToString());
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                Label1.Text += "<a" + (Request["State"] != null && Request["State"] == dt.Rows[i]["State"].ToString() ? " style='text-decoration:underline;background-color:#ccc;'" : "") + " href='?State=" + Convert.ToInt32(dt.Rows[i]["State"]) + "'>" + WX.WorkOrder.Order.StateStr[Convert.ToInt32(dt.Rows[i]["State"])] + "(" + dt.Rows[i]["scount"] + ")</a>&nbsp;&nbsp;&nbsp;&nbsp;";
            }
            for (int i = 0; i < dt2.Rows.Count; i++)
            {
                Label1.Text += "<a href='?State=" + Convert.ToInt32(dt2.Rows[i]["State"]) + "'>" + WX.WorkOrder.Order.StateStr[Convert.ToInt32(dt2.Rows[i]["State"])] + "(" + dt2.Rows[i]["scount"] + ")</a>&nbsp;&nbsp;&nbsp;&nbsp;";
            }
            string sSql = "select worder.* from WorkOrder_Dept wdept left join WorkOrder_Orders worder on wdept.WID=worder.ID where worder.State>0 and wdept.DeptID=" + this.CurUser.UserModel.DepartmentID.ToString();
            if (Request["State"] != null)
            {
                if (Convert.ToInt32(Request["State"]) < 3 || Request["State"]=="7")
                    sSql += " and wdept.State=" + Request["State"];
                else
                {
                    sSql = "select *,State DeptState from WorkOrder_Orders where ID in(select distinct porder.ID from WorkOrder_Orders corder left join WorkOrder_Orders porder on corder.PID=porder.ID where porder.State>0 and corder.DeptWorkID=" + this.CurUser.UserModel.DepartmentID.ToString() + " and corder.State=" + Request["State"] + ")";
                }
            }
            if (start)
            {
        ...
        public string Getchilds(int Id)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    bodystr += "<tr style=\"display:none;\" class=\"item" + Id + "\"><td colspan='2'><div style='padding-left:50px;" + mydept + "'><b>└ " + dt.Rows[i]["DeptName"] + "</b></div></td><td>&nbsp;</td><td>" + Convert.ToDateTime(dt.Rows[i]["AddTime"]).ToString("MM-dd HH:mm") + "</td><td>" + GetTimeimg(dt.Rows[i]["SubTime"].ToString(), "", 1, Id, 2, 0) + "</td><td>" + GetTimeimg(dt.Rows[i]["FPTime"].ToString(), dt.Rows[i]["SubTime"].ToString(), 2, Id, 2, Convert.ToInt32(dt.Rows[i]["DeptID"].ToString())) + "</td><td>" + GetTimeimg(dt.Rows[i]["YSTime"].ToString(), dt.Rows[i]["FPTime"].ToString(), 3, Id, 2, Convert.ToInt32(dt.Rows[i]["DeptID"].ToString())) + "</td><td>" + GetTimeimg(dt.Rows[i]["StopTime"].ToString(), dt.Rows[i]["YSTime"].ToString(), 4, Id, 2, 0) + "</td><td class=\"state" + dt.Rows[i]["State"] + "\">" + WX.WorkOrder.Order.StateStr[Convert.ToInt32(dt.Rows[i]["State"])] + "</td><td>" + (dt.Rows[i]["DeptID"].ToString() == this.CurUser.UserModel.DepartmentID.ToString() ? "<a href=javascript:PopupIFrame('WorkOrder_Fenpei.aspx?OrderID=" + Id + "','任务分配','','',850,450)>分配</a>" : "") + "</td></tr>";
                    for (int j = 0; j < dt2.Rows.Count; j++)
                    {
                        bodystr += "<tr style=\"display:none;\" class=\"item" + Id + "\"><td colspan='2'><div style='padding-left:100px;'>├ <a style='" + mydept + "' href=\"javascript:PopupIFrame('WorkOrder_Assign_Show.aspx?OrderID=" + dt2.Rows[j]["ID"] + "','查看任务','','',850,550)\">" + WX.CommonUtils.GetRealNameListByUserIdList(dt2.Rows[j]["ExecUserID"].ToString()) + "&nbsp;：&nbsp;" + dt2.Rows[j]["Title"] + (Convert.ToInt32(dt2.Rows[j]["mescount"]) > 0 ? "<img src='/images/4.gif' alt='您有新消息'/>" : "") + "</a></div></td><td>&nbsp;</td><td>" + Convert.ToDateTime(dt2.Rows[j]["AddTime"]).ToString("MM-dd HH:mm") + "</td><td>" + GetTimeimg(dt2.Rows[j]["SubTime"].ToString(), dt2.Rows[j]["AddTime"].ToString(), 1, Convert.ToInt32(dt2.Rows[j]["ID"].ToString()), 3, 0) + "</td><td>" + GetTimeimg(dt2.Rows[j]["FPTime"].ToString(), dt2.Rows[j]["FPTime"].ToString(), 2, Convert.ToInt32(dt2.Rows[j]["ID"].ToString()), 3, 0) + "</td><td>" + GetTimeimg(dt2.Rows[j]["YSTime"].ToString(), dt2.Rows[j]["SubTime"].ToString(), 3, Convert.ToInt32(dt2.Rows[j]["ID"].ToString()), 3, 0) + "</td><td>" + GetTimeimg(dt2.Rows[j]["StopTime"].ToString(), dt2.Rows[j]["YSTime"].ToString(), 4, Convert.ToInt32(dt2.Rows[j]["ID"].ToString()), 3, 0) + "</td><td class=\"state" + dt2.Rows[j]["State"] + "\">" + WX.WorkOrder.Order.StateStr[Convert.ToInt32(dt2.Rows[j]["State"])] + "</td><td>&nbsp;</td></tr>";
                    }
                }
            }
            return bodystr;
