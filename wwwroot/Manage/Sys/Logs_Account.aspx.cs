...
{
    public partial class Logs_Account : System.Web.UI.Page
    {
        ...
        private void InitComponent(bool start)
        {
            ...
            string sql = String.Format("SELECT log.*,emp.RealName FROM TL_AccountLogs log left join TU_Users emp on log.UserID=emp.UserID where 1=1{0}{1}{2}", key_con, begin_Con, end_Con);
            //Response.Write(sql);
            if (start)
            {
