            ...
            else
            {
                ...
                for (int i = 0; i < iTotal; i++)
                {
                    ...
                    if (file.ContentLength > 0 || !string.IsNullOrEmpty(file.FileName))
                    {
                        _count++;
                        //保存文件
                        file.SaveAs(System.Web.HttpContext.Current.Server.MapPath("~/UploadFiles/test/" + filename));

                        //新文件名
                        _newFileName = "UploadFiles/test/" + filename;
                        int type = 0;
                        int subId = 0;
                        Guid uploadUserId = Guid.NewGuid();
                        DateTime uploadTime = DateTime.Now;
                        string uploadIp = context.Request.UserHostAddress;
                        string cmdText = "INSERT INTO TE_Attaches (Type,SubId,NewFileName,OldFileName,UploadUserID,UploadTime,UploadIP) VALUES (@Type,@SubId,@NewFileName,@OldFileName,@UploadUserID,@UploadTime,@UploadIP)";
                        using (SqlConnection connection = new SqlConnection(connectionString))
                        {
                            connection.Open();
                            SqlCommand command = new SqlCommand(cmdText, connection);
                            command.Parameters.Add("@Type", SqlDbType.TinyInt, 1).Value = type;
                            command.Parameters.Add("@SubId", SqlDbType.BigInt, 8).Value = subId;
