<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SelectPeopleByFilter.aspx.cs" Inherits="wwwroot.App_Ctrl.SelectPeopleByFilter" ClientIDMode="Static" %>
...
<script type="text/javascript">
    $(function () {
        $('#btnSubmit').click(function () {
            ...
            var text = new Array();
            if (length == 0) {
                $('#' + outputField, window.parent.document).html("");
                window.parent.document.getElementById("dialogCase").style.display = 'none';
            } else if (length == 1) {
                $('#' + outputField, window.parent.document).html("");
                $('#' + hiddenField, window.parent.document).val("");
                var selectedText = $('#lbRight').text();
                var selectedValue = $('#lbRight').val();
                var innerHtml = "<span id='a" + selectedValue + "'>" + selectedText + "<a href=\"javascript:RemoveItem1('" + selectedValue + "');\"><img src='../images/remove.png'/></a></span>";
                $('#' + outputField, window.parent.document).append(innerHtml);
                $('#' + hiddenField, window.parent.document).val(selectedValue);
                window.parent.document.getElementById("dialogCase").style.display = 'none';
            } else {
                $('#' + outputField, window.parent.document).html("");
                $('#' + hiddenField, window.parent.document).val("");
                $('#lbRight option').each(function (i, selected) {
                    ...
                    var innerHtml = "<span id='b" + selectedValue + "'>" + selectedText + "<a href=\"javascript:RemoveItem2('" + selectedValue + "');\"><img src='../images/remove.png'/></a></span>";
                    $('#' + outputField, window.parent.document).append(innerHtml);
                });
                $('#' + hiddenField, window.parent.document).val(value);
                window.parent.document.getElementById("dialogCase").style.display = 'none';
            }
