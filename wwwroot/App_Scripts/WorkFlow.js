...
function showContextMenu(event, type) {
    ...
    var Flow_ID = objRect.getAttribute("flowId");

    var sMenu = style;
    sMenu += getMenuRow("Add_Process()", "新建借点"); height += 20;
    sMenu += getMenuRow("Del_Process(" + Process_ID + ")", "删除此借点"); height += 20;
    sMenu += getMenuRow("Edit_Process(" + Process_ID + ")", "节点属性"); height += 20;
    sMenu += sMenuHr;height += 2;
    sMenu += getMenuRow("EditP_opSet(" + Process_ID + ")", "经办设置"); height += 20;
    sMenu += getMenuRow("EditP_passSet(" + Process_ID + ")", "流转设置"); height += 20;
    sMenu += getMenuRow("EditP_editableFlds(" + Process_ID + ")", "可写字段"); height += 20;
    sMenu += getMenuRow("EditP_hiddenFlds(" + Process_ID + ")", "隐藏字段"); height += 20;
    sMenu += sMenuHr; height += 2;
    sMenu += getMenuRow("EditP_condition(" + Process_ID + ")", "条件设置"); height += 20;
    sMenu += getMenuRow("EditP_plugs(" + Process_ID + ")", "输入输出插件"); height += 20;
    /*
    //switch (type) {
    //    case 1:

    sMenu += getMenuRow("Edit_Process(" + Process_ID + "," + Flow_ID + ")", "节点属性");
    height += 20;
    ...
    sMenu += getMenuRow("Del_Process(" + Process_ID + ")", "添加借点");
    height += 20;
    //    break;

    //case 2:

    //    sMenu += getMenuRow("Add_Process()", "新建借点");
    //    height += 20;
...
function SavePosition() {
    ...
    for (var i = 0; i < a.length; i++) {
        ...
        if (table_id > 0) {
            strSql += "SET LEFT=" + mf_pixel_left + ",SETTOP=" + mf_pixel_top + " where ID=" + table_id + ";";
        }
    }
